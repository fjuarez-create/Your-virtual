...
| Modelo fuente | SketchUp 2026 en el sobremesa de Fran, con el exportador Datasmith **5.8** (`5_8_200`) instalado. El `.skp` del 17-sep es `SERENEA_Apolo_17.09.26.skp`, y está en la carpeta de copia de arriba. | **La web va con el mismo modelo desde el 18-sep**: `SERENEA_Apolo_17.09.glb` (release `modelos-v1` del repositorio), pasado por `tools/build_serenea.mjs`; los 32 cajones de corte y los 166 contornos de vivienda salieron iguales que con el v6, así que la calibración del marco no cambia.

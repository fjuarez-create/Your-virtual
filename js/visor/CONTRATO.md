# Contrato de módulos del visor (js/visor)

Motor nuevo del digital twin de SERENEA. Sustituye a `main.js antiguo` (que se
...
- ES modules sin bundler. Importaciones por el mapa de `index.html`:
  `three`, `three/addons/…`, `three-mesh-bvh`, `three-bvh-csg`,
  `three-gpu-pathtracer`, `camera-controls`, y `app/…` para los módulos
  existentes. Todo está en `vendor/` (Three.js r185).
...
- Cada módulo trae una página de demostración en `_demos/<módulo>.html`
...
- Tabla `MOMENTOS` heredada de `main.js antiguo` (elev, azim, turbidez,
...
(capa 1) sin post, como en `main.js antiguo` (bloque final de `loop`).
...
- Lo que queda por debajo de la planta activa se atenúa como hoy: menos
  ...
  inferiores (ver `animateFloors` en `main.js antiguo`), y la losa de la planta
...
`window.apolo` lo que usa `js/shell.js`:
...
Unreal (`window.__mensajes`, `window.__simularEvento`). `unreal.html` es
...
- Servidor: `python3 -m http.server 8765` desde la raíz del repositorio
  ...
  "%{http_code}" http://127.0.0.1:8765/index.html`).
...
<script type="importmap">{"imports":{
 ...
 "three-gpu-pathtracer":"/vendor/three-gpu-pathtracer.js",
 "camera-controls":"/vendor/camera-controls.js",
 "xatlas-web":"/vendor/xatlas-web.js",
 "app/":"/js/"
}}</script>
```

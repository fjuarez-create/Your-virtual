...
const GLSL_ATENUACION = /* glsl */`
  ...
  /* Vivienda abierta: no se pinta nada por encima de SU cota de corte. El
     edificio está escalonado y cada cajón se corta 1,35 m sobre su propio
     suelo, así que, mirando una vivienda desde dentro, los muros del cajón
     vecino más alto (y el medianero, que el fichero precortado corta por el
     cajón donde cae el centro de cada triángulo) asoman hasta 1,5 m por
     encima: «trozos cortados que no deberían verse». Dentro de su caja se
     recorta a su cota; fuera, los cajones más altos se rebajan hasta ella
     pero nunca por debajo de su propio suelo, para que queden como losas con
     el arranque de los muros y no como un agujero. */
  bool recortadoCorte(vec3 p) {
    if (p.y <= uRecorteVivY) return false;
    if (p.x > uRecorteViv.x && p.x < uRecorteViv.z && p.z > uRecorteViv.y && p.z < uRecorteViv.w) return true;
    return p.y > uSuelos[celdaCorte(p)] + 0.08;
  }
...
export function crearCortes(ctx, edificio, opciones = {}) {
  ...
  Object.assign(cortes, {
    ...
    /* Vivienda abierta: no se pinta nada que asome por encima de `cota`
       dentro de `caja` (con holgura para los muros de borde). `null` lo quita.

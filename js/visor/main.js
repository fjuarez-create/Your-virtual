...
/* Vivienda abierta (se ha llegado dentro, o null al salir): sin prisma ni
   cartela, el vidrio apenas teñido para que se vea como vidrio, y sin nada
   que asome por encima de su cota de corte dentro de su caja (los muros
   medianeros con el cajón vecino, más alto; ver cortes.setRecorteVivienda). */
function abrirVivienda(id) {
  if (!edificio) return;
  edificio.setViviendaAbierta(id);
  const v = id != null ? edificio.viviendas.get(id) : null;
  if (cortes) cortes.setRecorteVivienda(v ? v.caja : null, v ? cortes.alturas?.[v.plataforma] : null);
  repintar();
}
...
Object.assign(apolo, {
  ...
  select(id, { enfocar = true } = {}) {
    ...
    if (id !== apolo.selected) abrirVivienda(null); // otra vivienda: vuelve el prisma
